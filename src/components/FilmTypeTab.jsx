...
`;
export default () => {
    const LinkFilm = useParams();
    return (
        <FilmTypeTabs defaultActiveKey="2d" id="uncontrolled-tab-example" className="date-list">
            <Tab eventKey="2d" title="2D">
                <CinemaList>
                    <Cinema>
                        <CinemaTitle>Vincom Trần Duy Hưng</CinemaTitle>
                        <FilmTimeList>
                            <FilmTime to={`/booking-seat/film/${LinkFilm.id}`}>15:30</FilmTime>
                            <FilmTime to={`/booking-seat/film/${LinkFilm.id}`}>18:30</FilmTime>
                        </FilmTimeList>
                    </Cinema>
                </CinemaList>
            </Tab>
            <Tab eventKey="3d" title="3D">
                <CinemaList>
                    <Cinema>
                        ...
                        <FilmTimeList>
                            <FilmTime to={`/booking-seat/film/${LinkFilm.id}`}>16:15</FilmTime>
                            <FilmTime to={`/booking-seat/film/${LinkFilm.id}`}>18:45</FilmTime>
                            <FilmTime to={`/booking-seat/film/${LinkFilm.id}`}>22:30</FilmTime>
                        </FilmTimeList>
                    </Cinema>
